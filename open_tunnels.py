    ...
    try:
        with open(filename, "r") as pid_file:
            for line in pid_file:
                ...
                print(
                    f"Tunnel PID: {pid}, Host: {hostname}, User: {username}, Local Port: {port}, Status: {active}"
                )
    except FileNotFoundError:
        ...
        for tunnel in tunnels:
            ...
            process = subprocess.Popen(
                [
                    ...
                    "-N",
                    "-L",
                    f"{theport}:localhost:{theport}",
                    f"{theusername}@{thehostname}",
                ]
            )
            pid = process.pid
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "action",
        choices=["open", "kill", "list"],
        help="Action to perform: open, kill, or list tunnels",
    )
    parser.add_argument(
        "--pid-file",
        default=".open_tunnel_pids",
        help="Path to PID file (default: .open_tunnel_pids)",
    )
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Show tunnels in GUI window (use with list action)",
    )
