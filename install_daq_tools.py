...
def main():
    parser = argparse.ArgumentParser(description="Parse YAML file and extract DAQ tool paths.")
    parser.add_argument("-v", "--version", action="version", version="%(prog)s 1.0")
    parser.add_argument("-O",'--override', action='store_true', help="Override fatal errors and continue.")   
    parser.add_argument("-M", '--makedirs', action='store_true', help="Create missing install directories.")
    parser.add_argument("-f",'--yaml_file', type=str, default=default_release_file, help="Path to the YAML configuration file.")
    ...
    if not os.path.exists(base_release['path']) or not os.path.exists(test_release['path']):
