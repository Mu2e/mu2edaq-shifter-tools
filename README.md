...
Notes:
* `setup_online [partition] <env>` Set up the test release for the given environment, or if no option is provided set up the "main" or gateway instance
...
```
* `kill_daq [parition] <all>` Kills the ots instance associated with the current environment, or possibly kills all running ots environments
