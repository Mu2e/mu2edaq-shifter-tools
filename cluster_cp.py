...
parser = argparse.ArgumentParser(description="Copy files to a location across the full DAQ cluster.")
parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose output.")
parser.add_argument("-c","--calo", action="store_true", help="Copy files to the Calo nodes.")
parser.add_argument("-C","--crv", action="store_true", help="Copy files to the CRV nodes.")
parser.add_argument("-t","--trk", action="store_true", help="Copy files to the Trk nodes.")
parser.add_argument("-d","--daq", action="store_true", help="Copy files to the DAQ nodes.")
...
parser.add_argument("destination", nargs=1,  help="Destination path on the remote nodes.")
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    for f in files:
        for h in hosts:
            ...
            futures.append(executor.submit(copy_file_to_host, f, h, user, destination[0]))
